...
$originLat = 47.397971
$originLon = 8.546164
$stationEast = $StationUavHome
$stationWest = $StationUgvHome
$packageOriginLon = 8.549485
$packageDestLon = 8.542843
$stationEastLon = 8.54725
$stationWestLon = 8.54505
...
Write-Output "Registering scenario 2 dual-UGV network..."
Invoke-Api -Method POST -Path "/api/stations" -Admin -Body @{
    id = $stationEast
    name = "East Hub (pickup side)"
    position = @{ latitude = $originLat; longitude = $stationEastLon; altitudeMeters = 0.0 }
    storageCapacity = 4
    parkingCapacity = 2
} | Out-Null
...
Invoke-Api -Method POST -Path "/api/stations" -Admin -Body @{
    id = $stationWest
    name = "West Hub (delivery side)"
    position = @{ latitude = $originLat; longitude = $stationWestLon; altitudeMeters = 0.0 }
    storageCapacity = 4
    parkingCapacity = 2
} | Out-Null
...
Invoke-Api -Method POST -Path "/api/agents" -Body @{
    ...
    currentStationId = $stationEast
    runtime = @{ autoStart = $true; px4Model = "r1_rover"; px4Instance = 1; mavlinkPort = 14581; spawnInGazebo = $true }
} | Out-Null
Invoke-Api -Method POST -Path "/api/agents/$UgvEast/activate" | Out-Null
...
Invoke-Api -Method POST -Path "/api/agents" -Body @{
    ...
    maxRangeMeters = $ugvMaxRangeMeters
    payloadCapacityKg = 20.0
    currentStationId = $stationWest
    runtime = @{ autoStart = $true; px4Model = "r1_rover"; px4Instance = 2; mavlinkPort = 14582; spawnInGazebo = $true }
} | Out-Null
Invoke-Api -Method POST -Path "/api/agents/$UgvWest/activate" | Out-Null

$shipment = Invoke-Api -Method POST -Path "/api/shipments" -Body @{
    customerId = $CustomerScenario2DualUgv
    senderName = "Scenario2 Dual-UGV Sender"
    recipientName = "Scenario2 Dual-UGV Recipient"
...
Write-Output "Scenario 2 dual-UGV started. Shipment ID: $shipmentId"
Write-Output "Monitor log: $(Join-Path $runtimeDir 'scenario-2-dual-ugv-monitor.log')"

if ($StartMonitor -and -not [string]::IsNullOrWhiteSpace($shipmentId)) {
    $monitorLog = Join-Path $runtimeDir "scenario-2-dual-ugv-monitor.log"
