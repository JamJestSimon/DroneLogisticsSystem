...
Write-Output "Registering scenario 2c (single parking per station)..."
...
$shipment = Invoke-Api -Method POST -Path "/api/shipments" -Body @{
    customerId = $CustomerScenario2SingleParking
    senderName = "Scenario2 Single Parking Sender"
    recipientName = "Scenario2 Single Parking Recipient"
...
Write-Output "Scenario 2c started. Shipment ID: $shipmentId"
Write-Output "Parking: $parkingCapacity slot per station (UAV and UGV must swap hubs)."
Write-Output "Monitor log: $(Join-Path $runtimeDir 'scenario-2-single-parking-monitor.log')"

if ($StartMonitor -and -not [string]::IsNullOrWhiteSpace($shipmentId)) {
    $monitorLog = Join-Path $runtimeDir "scenario-2-single-parking-monitor.log"
